...
import (
	...
	"github.com/fzkun/gorm-dm8/clauses"
						...
						case reflect.Slice, reflect.Array:
							...
							if stmt.Schema.PrioritizedPrimaryField.HasDefaultValue {
								...
								case reflect.Struct:
									...
									_, isZero := stmt.Schema.PrioritizedPrimaryField.ValueOf(db.Statement.Context, insertTo)
									if isZero {
										stmt.Schema.PrioritizedPrimaryField.Set(db.Statement.Context, insertTo, insertID)
									}
								}
							}
						...
						if hasDefaultValues {
							...
							funk.ForEach(
								...
								func(field *gormSchema.Field) {
									switch insertTo.Kind() {
									case reflect.Struct:
										if err = field.Set(db.Statement.Context, insertTo, stmt.Vars[boundVars[field.Name]].(sql.Out).Dest); err != nil {
											db.AddError(err)
										}
									case reflect.Map:
										// TODO 设置id的值
